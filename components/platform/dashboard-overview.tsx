    ...
    <div className="space-y-4">
      ...
      {(() => {
        const pipelineStages = [
          ...
          { key: 'production', label: 'Em produção', count: scripts.filter(s => s.status === 'production').length, color: 'bg-amber-50 text-amber-700 border-amber-200', dot: 'bg-amber-500', href: `/${workspace}/posts?view=production` },
          { key: 'recording', label: 'Gravando', count: scripts.filter(s => s.status === 'recording').length, color: 'bg-rose-50 text-rose-700 border-rose-200', dot: 'bg-rose-500', href: `/${workspace}/posts?view=production` },
          { key: 'drive', label: 'No Drive', count: scripts.filter(s => s.status === 'drive').length, color: 'bg-sky-50 text-sky-700 border-sky-200', dot: 'bg-sky-500', href: `/${workspace}/posts?view=production` },
          { key: 'editing', label: 'Em edição', count: scripts.filter(s => s.status === 'editing').length, color: 'bg-purple-50 text-purple-700 border-purple-200', dot: 'bg-purple-500', href: `/${workspace}/posts?view=production` },
          { key: 'edited', label: 'Editado', count: scripts.filter(s => s.status === 'edited').length, color: 'bg-indigo-50 text-indigo-700 border-indigo-200', dot: 'bg-indigo-500', href: `/${workspace}/posts?view=edited` },
          { key: 'scheduled', label: 'Agendado', count: scripts.filter(s => s.status === 'scheduled').length, color: 'bg-blue-50 text-blue-700 border-blue-200', dot: 'bg-blue-500', href: `/${workspace}/posts?view=calendar` },
          { key: 'posted', label: 'Postado', count: scripts.filter(s => s.status === 'posted').length, color: 'bg-slate-100 text-slate-600 border-slate-200', dot: 'bg-slate-400', href: `/${workspace}/posts?view=posted` },
        ];
